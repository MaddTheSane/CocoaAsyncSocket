...
@property (nonatomic, readonly) uint8_t *readBuffer;

- (void)getReadBuffer:(uint8_t **)bufferPtr availableBytes:(size_t *)availableBytesPtr;

@property (nonatomic, readonly) size_t availableSpace;
@property (nonatomic, readonly) uint8_t *writeBuffer;
...
- (void)setDelegate:(id)newDelegate synchronously:(BOOL)synchronously
{
    ...
    dispatch_block_t block = ^{
        typeof(self) strongSelf = weakSelf;
		strongSelf.delegate = newDelegate;
	};
...
- (dispatch_queue_t)delegateQueue
{
	...
	else
	{
		...
		dispatch_sync(socketQueue, ^{
		typeof(self) strongSelf = weakSelf;
            result = strongSelf.delegateQueue;
		});
...
- (void)setDelegateQueue:(dispatch_queue_t)newDelegateQueue synchronously:(BOOL)synchronously
{
    ...
    dispatch_block_t block = ^{
		typeof(self) strongSelf = weakSelf;
		#if !OS_OBJECT_USE_OBJC
		if (strongSelf.delegateQueue) dispatch_release(strongSelf.delegateQueue);
		if (newDelegateQueue) dispatch_retain(newDelegateQueue);
		#endif
		
		strongSelf.delegateQueue = newDelegateQueue;
	};
...
- (void)getDelegate:(id *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	...
	else
	{
		...
		dispatch_sync(socketQueue, ^{
            typeof(self) strongSelf = weakSelf;
			dPtr = strongSelf.delegate;
			dqPtr = strongSelf.delegateQueue;
		});
...
- (BOOL)isIPv4Enabled
{
	...
	else
	{
		...
		dispatch_sync(socketQueue, ^{
            typeof(self) strongSelf = weakSelf;
			result = ((strongSelf->config & kIPv4Disabled) == 0);
		});
...
- (void)setIPv4Enabled:(BOOL)flag
{
	...
	dispatch_block_t block = ^{
		typeof(self) strongSelf = weakSelf;
		if (flag)
			strongSelf->config &= ~kIPv4Disabled;
		else
			strongSelf->config |= kIPv4Disabled;
	};
...
- (BOOL)isIPv6Enabled
{
	...
	else
	{
		...
		dispatch_sync(socketQueue, ^{
            typeof(self) strongSelf = weakSelf;
			result = ((strongSelf->config & kIPv6Disabled) == 0);
		});
...
- (void)setIPv6Enabled:(BOOL)flag
{
	...
	dispatch_block_t block = ^{
		typeof(self) strongSelf = weakSelf;
		if (flag)
			strongSelf->config &= ~kIPv6Disabled;
		else
			strongSelf->config |= kIPv6Disabled;
	};
...
- (BOOL)isIPv4PreferredOverIPv6
{
	...
	else
	{
		...
		dispatch_sync(socketQueue, ^{
            typeof(self) strongSelf = weakSelf;
			result = ((strongSelf->config & kPreferIPv6) == 0);
		});
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		typeof(self) strongSelf = weakSelf;
		if (strongSelf.delegate == nil) // Must have delegate set
		{
		...
		if (strongSelf.delegateQueue == NULL) // Must have delegate queue set
		{
		...
		BOOL isIPv4Disabled = (strongSelf->config & kIPv4Disabled) ? YES : NO;
		BOOL isIPv6Disabled = (strongSelf->config & kIPv6Disabled) ? YES : NO;
		...
		[strongSelf->readQueue removeAllObjects];
		[strongSelf->writeQueue removeAllObjects];
		...
		if (enableIPv4)
		{
			...
			strongSelf->socket4FD = createSocket(AF_INET, interface4);
			
			if (strongSelf->socket4FD == SOCKET_NULL)
			{
				return_from_block;
			}
		}
		...
		if (enableIPv6)
		{
			...
			strongSelf->socket6FD = createSocket(AF_INET6, interface6);
			
			if (strongSelf->socket6FD == SOCKET_NULL)
			{
				if (strongSelf->socket4FD != SOCKET_NULL)
				{
					LogVerbose(@"close(socket4FD)");
					close(strongSelf->socket4FD);
				}
		...
		if (enableIPv4)
		{
			strongSelf->accept4Source = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, strongSelf.socket4FD, 0, strongSelf->socketQueue);
			
			int socketFD = strongSelf.socket4FD;
			dispatch_source_t acceptSource = strongSelf->accept4Source;
			
			__weak GCDAsyncSocket *weakerSelf = self;
			
			dispatch_source_set_event_handler(strongSelf->accept4Source, ^{ @autoreleasepool {
				
				__strong GCDAsyncSocket *strongerSelf = weakerSelf;
				if (strongerSelf == nil) return_from_block;
				...
				while ([strongerSelf doAccept:socketFD] && (++i < numPendingConnections));
				
			}});
			
			
			dispatch_source_set_cancel_handler(strongSelf->accept4Source, ^{
			...
			dispatch_resume(strongSelf->accept4Source);
		}
		
		if (enableIPv6)
		{
			strongSelf->accept6Source = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, strongSelf.socket6FD, 0, strongSelf->socketQueue);
			
			int socketFD = strongSelf.socket6FD;
			dispatch_source_t acceptSource = strongSelf->accept6Source;
			
			__weak GCDAsyncSocket *weakerSelf = self;
			
			dispatch_source_set_event_handler(strongSelf->accept6Source, ^{ @autoreleasepool {
			...
			dispatch_source_set_cancel_handler(strongSelf->accept6Source, ^{
			...
			dispatch_resume(strongSelf->accept6Source);
		}
		
		strongSelf->flags |= kSocketStarted;
                ...
                error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		int aStateIndex = strongSelf->stateIndex;
		__weak GCDAsyncSocket *weakerSelf = self;
		...
		dispatch_async(globalConcurrentQueue, ^{ @autoreleasepool {
			...
			if (lookupErr)
			{
				dispatch_async(strongerSelf->socketQueue, ^{ @autoreleasepool {
					
					[strongerSelf lookup:aStateIndex didFail:lookupErr];
				}});
			}
			else
			{
				...
				dispatch_async(strongSelf->socketQueue, ^{ @autoreleasepool {
					...
					[strongerSelf lookup:aStateIndex didSucceedWithAddress4:address4 address6:address6];
				}});
			}
...
- (void)didConnect:(int)aStateIndex
{
	...
	if (delegateQueue && [theDelegate respondsToSelector:@selector(socket:didConnectToHost:port:)])
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			typeof(self) strongSelf = weakSelf;
			[theDelegate socket:strongSelf didConnectToHost:host port:port];
...
- (void)disconnect
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		typeof(self) strongSelf = weakSelf;
		if (strongSelf->flags & kSocketStarted)
		{
...
- (void)disconnectAfterReading
{
	...
	dispatch_async(socketQueue, ^{ @autoreleasepool {
		...
		if (strongSelf->flags & kSocketStarted)
		{
			strongSelf->flags |= (kForbidReadsWrites | kDisconnectAfterReads);
			[strongSelf maybeClose];
		}
	}});
}
...
- (void)disconnectAfterWriting
{
	...
	dispatch_async(socketQueue, ^{ @autoreleasepool {
		...
		if (strongSelf->flags & kSocketStarted)
		{
			strongSelf->flags |= (kForbidReadsWrites | kDisconnectAfterWrites);
			[strongSelf maybeClose];
		}
	}});
}
...
- (void)disconnectAfterReadingAndWriting
{
	...
	dispatch_async(socketQueue, ^{ @autoreleasepool {
		...
		if (strongSelf->flags & kSocketStarted)
		{
			strongSelf->flags |= (kForbidReadsWrites | kDisconnectAfterReads | kDisconnectAfterWrites);
			[strongSelf maybeClose];
		}
	}});
}
...
- (BOOL)isDisconnected
{
	...
	dispatch_block_t block = ^{
        typeof(self) strongSelf = weakSelf;
		result = (strongSelf->flags & kSocketStarted) ? NO : YES;
	};
...
- (BOOL)isConnected
{
	...
	dispatch_block_t block = ^{
        typeof(self) strongSelf = weakSelf;
		result = (strongSelf->flags & kConnected) ? YES : NO;
	};
...
- (NSString *)connectedHost
{
	...
	else
	{
		...
		dispatch_sync(socketQueue, ^{ @autoreleasepool {
			typeof(self) strongSelf = weakSelf;
			if (strongSelf->socket4FD != SOCKET_NULL)
				result = [strongSelf connectedHostFromSocket4:strongSelf->socket4FD];
			else if (strongSelf->socket6FD != SOCKET_NULL)
				result = [strongSelf connectedHostFromSocket6:strongSelf->socket6FD];
		}});
...
- (uint16_t)connectedPort
{
	...
	else
	{
		...
		dispatch_sync(socketQueue, ^{
			...
			if (strongSelf->socket4FD != SOCKET_NULL)
				result = [strongSelf connectedPortFromSocket4:strongSelf->socket4FD];
			else if (strongSelf->socket6FD != SOCKET_NULL)
				result = [strongSelf connectedPortFromSocket6:strongSelf->socket6FD];
		});
...
- (NSString *)localHost
{
	...
	else
	{
		...
		dispatch_sync(socketQueue, ^{ @autoreleasepool {
			typeof(self) strongSelf = weakSelf;
			if (strongSelf->socket4FD != SOCKET_NULL)
				result = [strongSelf localHostFromSocket4:strongSelf->socket4FD];
			else if (strongSelf->socket6FD != SOCKET_NULL)
				result = [strongSelf localHostFromSocket6:strongSelf->socket6FD];
		}});
...
- (uint16_t)localPort
{
	...
	else
	{
		...
		dispatch_sync(socketQueue, ^{
			...
			typeof(self) strongSelf = weakSelf;
			if (strongSelf->socket4FD != SOCKET_NULL)
				result = [strongSelf localPortFromSocket4:strongSelf->socket4FD];
			else if (strongSelf->socket6FD != SOCKET_NULL)
				result = [strongSelf localPortFromSocket6:strongSelf->socket6FD];
		});
...
- (NSData *)connectedAddress
{
	...
	dispatch_block_t block = ^{
		...
		if (strongSelf->socket4FD != SOCKET_NULL)
		{
			...
			if (getpeername(strongSelf->socket4FD, (struct sockaddr *)&sockaddr4, &sockaddr4len) == 0)
			{
		...
		if (strongSelf->socket6FD != SOCKET_NULL)
		{
			...
			if (getpeername(strongSelf->socket6FD, (struct sockaddr *)&sockaddr6, &sockaddr6len) == 0)
			{
...
- (NSData *)localAddress
{
	...
	dispatch_block_t block = ^{
		...
		if (strongSelf->socket4FD != SOCKET_NULL)
		{
			...
			if (getsockname(strongSelf->socket4FD, (struct sockaddr *)&sockaddr4, &sockaddr4len) == 0)
			{
				result = [[NSData alloc] initWithBytes:&sockaddr4 length:sockaddr4len];
			}
		}
		...
		if (strongSelf->socket6FD != SOCKET_NULL)
		{
			...
			if (getsockname(strongSelf->socket6FD, (struct sockaddr *)&sockaddr6, &sockaddr6len) == 0)
			{
				result = [[NSData alloc] initWithBytes:&sockaddr6 length:sockaddr6len];
			}
		}
...
- (BOOL)isIPv4
{
	...
	else
	{
		...
		dispatch_sync(socketQueue, ^{
            typeof(self) strongSelf = weakSelf;
			result = (strongSelf->socket4FD != SOCKET_NULL);
		});
...
- (BOOL)isIPv6
{
	...
	else
	{
		...
		dispatch_sync(socketQueue, ^{
            typeof(self) strongSelf = weakSelf;
			result = (strongSelf->socket6FD != SOCKET_NULL);
		});
...
- (BOOL)isSecure
{
	...
	else
	{
		...
		dispatch_sync(socketQueue, ^{
            typeof(self) strongSelf = weakSelf;
			result = (strongSelf->flags & kSocketSecure) ? YES : NO;
		});
                        ...
                        tag:(long)tag
{
	...
	dispatch_async(socketQueue, ^{ @autoreleasepool {
		...
		if ((strongSelf->flags & kSocketStarted) && !(strongSelf->flags & kForbidReadsWrites))
		{
			[strongSelf->readQueue addObject:packet];
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
	typeof(self) strongSelf = weakSelf;
		if (!strongSelf->currentRead || ![strongSelf->currentRead isKindOfClass:[GCDAsyncReadPacket class]])
		{
		...
		else
		{
			...
			NSUInteger done = strongSelf->currentRead->bytesDone;
			NSUInteger total = strongSelf->currentRead->readLength;
			
			if (tagPtr != NULL)   *tagPtr = strongSelf->currentRead->tag;
...
- (void)flushSSLBuffers
{
	...
	dispatch_block_t updateEstimatedBytesAvailable = ^{
		...
		estimatedBytesAvailable = strongSelf->socketFDBytesAvailable + [strongSelf->sslPreBuffer availableBytes];
		
		size_t sslInternalBufSize = 0;
		SSLGetBufferedReadSize(strongSelf->sslContext, &sslInternalBufSize);
...
- (void)doReadTimeout
{
	...
	if (delegateQueue && [theDelegate respondsToSelector:@selector(socket:shouldTimeoutReadWithTag:elapsed:bytesDone:)])
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			timeoutExtension = [theDelegate socket:strongSelf shouldTimeoutReadWithTag:theRead->tag
			                                                             elapsed:theRead->timeout
			                                                           bytesDone:theRead->bytesDone];
			
			dispatch_async(strongSelf->socketQueue, ^{ @autoreleasepool {
				
				[strongSelf doReadTimeoutWithExtension:timeoutExtension];
			}});
		}});
	}
	else
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	dispatch_async(socketQueue, ^{ @autoreleasepool {
		...
		if ((strongSelf->flags & kSocketStarted) && !(strongSelf->flags & kForbidReadsWrites))
		{
			[strongSelf->writeQueue addObject:packet];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		typeof(self) strongSelf = weakSelf;
		if (!strongSelf->currentWrite || ![strongSelf->currentWrite isKindOfClass:[GCDAsyncWritePacket class]])
		{
		...
		{
			NSUInteger done = strongSelf->currentWrite->bytesDone;
			NSUInteger total = [strongSelf->currentWrite->buffer length];
			
			if (tagPtr != NULL)   *tagPtr = strongSelf->currentWrite->tag;
			if (donePtr != NULL)  *donePtr = done;
			if (totalPtr != NULL) *totalPtr = total;
	...
	if (delegateQueue && [theDelegate respondsToSelector:@selector(socket:shouldTimeoutWriteWithTag:elapsed:bytesDone:)])
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			dispatch_async(strongSelf->socketQueue, ^{ @autoreleasepool {
				...
				[strongSelf doWriteTimeoutWithExtension:timeoutExtension];
			}});
		}});
	}
	else
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	dispatch_async(socketQueue, ^{ @autoreleasepool {
		typeof(self) strongSelf = weakSelf;
		if ((strongSelf->flags & kSocketStarted) && !(strongSelf->flags & kQueuedTLS) && !(strongSelf->flags & kForbidReadsWrites))
		{
			[strongSelf->readQueue addObject:packet];
			[strongSelf->writeQueue addObject:packet];
			
			strongSelf->flags |= kQueuedTLS;
...
- (BOOL)autoDisconnectOnClosedReadStream
{
	...
	else
	{
		...
		dispatch_sync(socketQueue, ^{
            typeof(self) strongSelf = weakSelf;
			result = ((strongSelf->config & kAllowHalfDuplexConnection) == 0);
		});
...
- (void)setAutoDisconnectOnClosedReadStream:(BOOL)flag
{
	...
	dispatch_block_t block = ^{
		typeof(self) strongSelf = weakSelf;
		if (flag)
			strongSelf->config &= ~kAllowHalfDuplexConnection;
		else
			strongSelf->config |= kAllowHalfDuplexConnection;
	};
