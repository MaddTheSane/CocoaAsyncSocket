...
typedef NS_ENUM(NSUInteger, GCDAsyncSocketFlags)
{
...
- (void)ensureCapacityForWrite:(size_t)numBytes
{
	...
	if (numBytes > availableSpace)
	{
		...
		size_t readPointerOffset = (size_t)(readPointer - preBuffer);
		size_t writePointerOffset = (size_t)(writePointer - preBuffer);
...
- (size_t)availableBytes
{
	return (size_t)(writePointer - readPointer);
}
...
- (size_t)availableSpace
{
	return (size_t)(preBufferSize - (size_t)(writePointer - preBuffer));
}
...
- (NSUInteger)readLengthForTermWithPreBuffer:(GCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr
{
	...
	for (i = 0; i < loopCount; i++)
	{
		...
		else
		{
			...
			{
				NSUInteger preOffset = (NSUInteger)(pre - [preBuffer readBuffer]); // pointer arithmetic
...
- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes
{
	...
	NSUInteger buffLength = (NSUInteger)(bytesDone + (unsigned long)numBytes);
	...
	NSUInteger i = ((buffLength - (unsigned long)numBytes) >= termLength) ? (buffLength - (unsigned long)numBytes - termLength + 1) : 0;
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			self->accept4Source = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, (uintptr_t)self->socket4FD, 0, self->socketQueue);
		...
		if (enableIPv6)
		{
			self->accept6Source = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, (uintptr_t)self->socket6FD, 0, self->socketQueue);
...
- (BOOL)acceptOnUrl:(NSURL *)url error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		self->acceptUNSource = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, (uintptr_t)self->socketUN, 0, self->socketQueue);
...
- (BOOL)connectToUrl:(NSURL *)url withTimeout:(NSTimeInterval)timeout error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (![self connectWithAddressUN:self->connectInterfaceUN error:&localErr])
		{
		...
		if (bytesRead > 0)
		{
			...
			else if (currentRead->term != nil)
			{
				...
				else
				{
					...
					NSInteger overflow = [currentRead searchForTermAfterPreBuffering:(ssize_t)bytesRead];
					...
					else if (overflow > 0)
					{
						...
						NSInteger underflow = (NSInteger)(bytesRead - (unsigned long)overflow);
						
						// Copy excess data into preBuffer
						
						LogVerbose(@"copying %ld overflow bytes into preBuffer", (long)overflow);
						[preBuffer ensureCapacityForWrite:(size_t)overflow];
						...
						currentRead->bytesDone += (unsigned long)underflow;
						totalBytesReadForCurrentRead += (unsigned long)underflow;
						done = YES;
					}
					else
					{
...
- (void)ssl_startTLS
{
	...
	if ([value isKindOfClass:[NSNumber class]])
	{
		status = SSLSetSessionOption(sslContext, kSSLSessionOptionFalseStart, (Boolean)[value boolValue]);
		if (status != noErr)
		{
	...
	if ([value isKindOfClass:[NSNumber class]])
	{
		status = SSLSetSessionOption(sslContext, kSSLSessionOptionSendOneByteRecord, (Boolean)[value boolValue]);
		if (status != noErr)
		{
			[self closeWithError:
	...
	if ([value isKindOfClass:[NSArray class]])
	{
		...
		for (cipherIndex = 0; cipherIndex < numberCiphers; cipherIndex++)
		{
			...
			ciphers[cipherIndex] = (SSLCipherSuite)[cipherObject shortValue];
		}
